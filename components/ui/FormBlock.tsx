...
export function FormBlock({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="space-y-5 [&_input]:w-full [&_input]:rounded [&_input]:border [&_input]:border-[var(--color-border)] [&_input]:bg-white [&_input]:px-4 [&_input]:py-3 [&_input]:text-sm [&_input]:text-[var(--color-text)] [&_input]:outline-none [&_input]:transition-colors [&_input]:placeholder:text-[var(--color-text-muted)] [&_input]:focus:border-[var(--color-text)] [&_label]:mb-1.5 [&_label]:block [&_label]:text-xs [&_label]:font-semibold [&_label]:uppercase [&_label]:tracking-wider [&_label]:text-[var(--color-text)] [&_select]:w-full [&_select]:appearance-none [&_select]:rounded [&_select]:border [&_select]:border-[var(--color-border)] [&_select]:bg-white [&_select]:px-4 [&_select]:py-3 [&_select]:text-sm [&_select]:text-[var(--color-text)] [&_select]:outline-none [&_select]:transition-colors [&_select]:focus:border-[var(--color-text)] [&_textarea]:w-full [&_textarea]:rounded [&_textarea]:border [&_textarea]:border-[var(--color-border)] [&_textarea]:bg-white [&_textarea]:px-4 [&_textarea]:py-3 [&_textarea]:text-sm [&_textarea]:text-[var(--color-text)] [&_textarea]:outline-none [&_textarea]:transition-colors [&_textarea]:placeholder:text-[var(--color-text-muted)] [&_textarea]:focus:border-[var(--color-text)]"
    >
      {children}
